...
import {  AiFillEye, AiFillLike, AiFillDislike,AiFillDelete,  } from 'react-icons/ai';
import { GrUpdate } from "react-icons/gr";
import Nav from './Navbar';
import { Navbar } from '@material-tailwind/react';
import Footer from './Footer';
import { useLocation } from "react-router-dom";
import { Blogurl, Upvoteurl, Downvoteurl,Commenturl,LikeCommenturl,DislikeCommenturl  } from "../src/index";
          ...
          <div className="max-w-4xl mx-auto py-12 px-12 lg:px-24 -mt-32 relative z-10">
            ...
            <div className="mt-4 mb-4">
              <div className="flex items-center ">
                <AiFillLike className="text-blue-500 cursor-pointer text-3xl" onClick={()=>{upvoteBlog(id.state)}}/>
                <span className="ml-3 mt-3 text-gray-600">
                  {blog.upVoteCount}
                </span>
                <AiFillDislike className="ml-4 text-blue-500 cursor-pointer text-3xl" onClick={()=>{downvoteBlog(id.state)}} />
                <span className="ml-3 mt-3 text-gray-600">
                  {blog.downVoteCount}
                </span>
            ...
            <div className="container mx-auto mt-8 px-4 sm:px-6 lg:px-8">
              <div className="comment-section">
                ...
                {getAllComment && getAllComment.map((comment, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-start space-x-4 mt-4">
                      ...
                      >
                        <div className='flex' >
                           <AiFillLike className="text-blue-500 cursor-pointer text-xl" onClick={()=>{LikeComment(comment.id)}}/>
                        <span className="ml-3 text-gray-600">{comment.likeCount}</span>
                        <AiFillDislike className="text-blue-500 cursor-pointer text-xl ml-4" onClick={()=>{DislikeComment(comment.id)}} />
                        <span className="ml-3  text-gray-600">{comment.disLikeCount}</span>
                        </div>

                       
                      </button>
                      {(blog.userId == comment.userId) &&(<>
                      <AiFillDelete className=" ml-5 text-blue-500 cursor-pointer text-xl" onClick={()=>{deleteComment(comment.id)}}/>
                      <GrUpdate className=" ml-5 text-blue-500 cursor-pointer text-lg" onClick={()=>{}}></GrUpdate>
                      </>)}
                ...
                >
                  <textarea
                    ...
                    rows="4"
                    onInput={(e)=>{setpostComment(e.target.value)}}
                  ></textarea>
                  <button  className="w-full text-white bg-indigo-500 hover:bg-indigo-600 rounded-md py-3 mt-4 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"  onClick={()=>{createComment(id.state)}} >
                    Post Comment
                  </button>
                </div>
              </div>
